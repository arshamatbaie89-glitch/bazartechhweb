...
export function checkDualRateLimit(ipKey, userKey, maxPerIp = 30, maxPerUser = 20, windowMs = 60000) {
  const ipOk = checkRateLimit(`ip:${ipKey}`, maxPerIp, windowMs)
  const userOk = userKey ? checkRateLimit(`user:${userKey}`, maxPerUser, windowMs) : true
  return ipOk && userOk
}
